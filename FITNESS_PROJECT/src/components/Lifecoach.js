  ...
  return (
    <div className="lc-page">
      ...
      <div className="lc-hero-section">
        ...
        />
        <h1 style={{ fontFamily: "'Permanent Marker', cursive" }}>Become the Best Version of Yourself</h1>
        <p>Fitness • Nutrition • Mindset</p>
      </div>

      {/* CONTENT */}
      <div className="lc-lifecoach-container">

        {/* MAIN CARDS */}
        <div className="lc-main-cards-container">
          <div
            className={`lc-main-card ${activeCard === 1 ? "active" : ""}`}
            onClick={() => setActiveCard(activeCard === 1 ? null : 1)}
            style={{ backgroundImage: `url(${fitnessImg})` }}
          >
            <h2>Fitness Training</h2>
            <p className="lc-card-short">Build strength and stay active</p>
            <div className="lc-card-details">
          ...
          <div
            className={`lc-main-card ${activeCard === 2 ? "active" : ""}`}
            onClick={() => setActiveCard(activeCard === 2 ? null : 2)}
            style={{ backgroundImage: `url(${nutritionImg})` }}
          >
            <h2>Healthy Nutrition</h2>
            <p className="lc-card-short">Fuel your body correctly</p>
            <div className="lc-card-details">
          ...
          <div
            className={`lc-main-card ${activeCard === 3 ? "active" : ""}`}
            onClick={() => setActiveCard(activeCard === 3 ? null : 3)}
            style={{ backgroundImage: `url(${mindsetImg})` }}
          >
            <h2>Strong Mindset</h2>
            <p className="lc-card-short">Stay disciplined and focused</p>
            <div className="lc-card-details">
        ...
        {/* BEST PRACTICES */}
        <h2 className="lc-section-title">Best Practices</h2>
        <div className="lc-cards-container">
          {Object.entries(cardData).map(([key, card]) => (
            <div
              className="lc-coach-card"
              key={key}
              style={{ backgroundImage: `url(${card.img})` }}
            >
              <h3>{card.title}</h3>
              <p className="lc-short">{card.subtitle}</p>
              <p className="lc-details">{card.description}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <footer className="lc-footer">
        ...
        <div className="lc-footer-bottom">© 2026 FITNESS PROJECT</div>
      </footer>
    </div>
  );
}
